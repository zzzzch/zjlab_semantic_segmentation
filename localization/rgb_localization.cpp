...
DEFINE_string(scene_path, "/home/zhachanghai/test_grap/pcl_cloud/",
              "scene pcd file");

DEFINE_string(target_path, "/home/zhachanghai/guochen_test/model.ply",
              "target ply file");

/*
 *  add coordinate axis in the image, use for show the pose info
 *  P.S : only for demo
 * */
...
int main(int argc, char **argv) {
    ...
    //    string guochen_movie_file =
    //    "/home/zhachanghai/guochen_test/dmrep_20200913_135548.oni";
    ...
    //    getVideoFrame2Cloud(guochen_movie_file, intrinsics_depth,
    //    output_path);
    ...
    for(int count = 0;count < 1; count++){
        ...
        //        "/home/zhachanghai/guochen_test/read_test.pcd";

        string target_file_path = FLAGS_target_path;

        LOG(INFO) << t.toc() << "Begin estimate pose !!";
...
//        LOG_EVERY_N(INFO,100) << "distance is " << distance ;
        ...
        LOG(INFO) << t.toc() << "read pcd file over !!";
        ...
        LOG(INFO) << t.toc() << "After downsample, scene point number is "
                  << scene->size();
        //        LOG(INFO) << t.toc() << "After downsample, target point number
        //        is " << model->size();
        //        grid.setInputCloud (model);
        //        grid.filter (*model);

        pcl::io::savePCDFileASCII("/home/zhachanghai/test_grap/down_scene.pcd",
                                  *scene);
        //        pcl::io::savePCDFileASCII("/home/zhachanghai/test_grap/down_target.pcd",
        //        *model);
        ...
        LOG(INFO) << t.toc()
                  << "After euclidean cluster, process scene point number is "
                  << process_scene->size();

        pcl::io::savePCDFileASCII(
            "/home/zhachanghai/test_grap/process_scene.pcd", *process_scene);
        ...
        LOG(INFO) << t.toc() << "down size target from " << model->size()
                  << " to " << cloud_src->size();
        ...
        LOG(INFO) << t.toc() << "src normal estimation over";
        ...
        //        LOG(INFO) << "down size scene from " << process_scene->size()
        //        << " to "<< cloud_tgt->size();
        ...
        LOG(INFO) << t.toc() << "target normal estimation over ";
        ...
        fpfh_src.setNumberOfThreads(6);
        //fpfh_src.setKSearch(20);
        fpfh_src.compute(*fpfhs_src);
        LOG(INFO) << t.toc() << "compute model cloud FPFH over omp!";

        //        //计算FPFH
        //        pcl::FPFHEstimation<pcl::PointXYZ,pcl::Normal,pcl::FPFHSignature33>
        //        fpfh_src1;
        //        fpfh_src1.setInputCloud(cloud_src);
        //        fpfh_src1.setInputNormals(cloud_src_normals);
        //        pcl::search::KdTree<pcl::PointXYZ>::Ptr tree_src_fpfh1 (new
        //        pcl::search::KdTree<pcl::PointXYZ>);
        //        fpfh_src1.setSearchMethod(tree_src_fpfh1);
        //        pcl::PointCloud<pcl::FPFHSignature33>::Ptr fpfhs_src1(new
        //        pcl::PointCloud<pcl::FPFHSignature33>());
        //        fpfh_src1.setRadiusSearch(0.05);
        //        //fpfh_src.setKSearch(20);
        //        fpfh_src1.compute(*fpfhs_src1);
        //        LOG(INFO) << t.toc() << "compute model cloud FPFH over
        //        origin!";
        ...
        fpfh_tgt.setNumberOfThreads(6);
        //fpfh_tgt.setKSearch(20);
        fpfh_tgt.compute(*fpfhs_tgt);
        LOG(INFO) << t.toc() << "compute target cloud FPFH over omp!";

        //        pcl::FPFHEstimation<pcl::PointXYZ,pcl::Normal,pcl::FPFHSignature33>
        //        fpfh_tgt1;
        //        fpfh_tgt1.setInputCloud(cloud_tgt);
        //        fpfh_tgt1.setInputNormals(cloud_tgt_normals);
        //        pcl::search::KdTree<pcl::PointXYZ>::Ptr tree_tgt_fpfh1 (new
        //        pcl::search::KdTree<pcl::PointXYZ>);
        //        fpfh_tgt1.setSearchMethod(tree_tgt_fpfh1);
        //        pcl::PointCloud<pcl::FPFHSignature33>::Ptr fpfhs_tgt1(new
        //        pcl::PointCloud<pcl::FPFHSignature33>());
        //        fpfh_tgt1.setRadiusSearch(0.05);
        //        //fpfh_tgt.setKSearch(20);
        //        fpfh_tgt1.compute(*fpfhs_tgt1);
        //        LOG(INFO) << t.toc() << "compute target cloud FPFH over
        //        origin!";

        LOG(INFO) << "cloud src size is " << cloud_src->size();
        LOG(INFO) << "cloud tgt size is " << cloud_tgt->size();
        //SAC配准
        ...
        LOG(INFO) << t.toc() << "sac has converged:" << scia.hasConverged()
                  << "  score: " << scia.getFitnessScore();
        Eigen::Matrix4f sac_trans;
        sac_trans = scia.getFinalTransformation();
        LOG(INFO) << sac_trans;
        //        pcl::io::savePCDFileASCII("/home/zhachanghai/bottle/icp_guess.pcd",
        //        *sac_result);
...
//    icp.setRANSACOutlierRejectionThreshold (1.5);
        ...
        //        to_string(count) + "_result_scene.pcd", *final_scene_);

        //        output << rotation (0,0) << " " << rotation (0,1) << " " <<
        //        rotation (0,2) << endl;
        //        output << rotation (1,0) << " " << rotation (1,1) << " " <<
        //        rotation (1,2) << endl;
        //        output << rotation (2,0) << " " << rotation (2,1) << " " <<
        //        rotation (2,2) << endl;
...
//        addLine(infile_pic, count, rototranslation);
        ...
        pcl::io::savePCDFileASCII("/home/zhachanghai/test_grap/result/" +
